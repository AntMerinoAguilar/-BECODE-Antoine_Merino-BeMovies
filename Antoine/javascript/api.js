...
let latestMovies;
let actionMovies;
let creditsForLatestMovies;
let creditsForActionMovies;
...
const getCreditsForMovies=async (movies)=>{
    let creditsMovies=[];
    for(let i=0;i<movies.length;i++){
        try {
            let resCreditsMovies=await fetch(`https://api.themoviedb.org/3/movie/${movies[i]["id"]}/credits`, options);
            let dataCreditsMovies=await resCreditsMovies.json();
            creditsMovies.push(dataCreditsMovies);
        } catch (error) {
        }
    }
...
const getData=async()=>{ 
    try {
        ...
        //associe les derniers films avec leurs crédits
        let creditsLatestMovies=await getCreditsForMovies(dataLatestMovies["results"]);
        let creditsActionMovies=await getCreditsForMovies(dataActionMovies["results"]);
        
        return {
            nowPlayingMovies: dataLatestMovies,
            genreMovies: dataGenre,
            actionMovies:dataActionMovies,
            creditsLatestMovies:creditsLatestMovies,
            creditsActionMovies:creditsActionMovies
        };
    } catch (error) {
        console.log(error)
    }
...
const addEventHover=(hover,movie,credits)=>{
...
const getGenresForMovie=(availableGenres,genreIdsMovie)=>{
    let strGenres=""
...
const getCastForMovie=(credits, idMovie)=>{
    let actors="";
    for(let i=0;i<credits.length;i++){
        if(idMovie==credits[i]["id"]){
            for(let j=0;j<5;j++){
                actors+=credits[i]["cast"][j]["name"]+(j==4?"":", ");
            }
        }
    }
    return actors;
...
const createContentHover=(movie)=>{
    ...
    h3.innerText=getGenresForMovie(availableGenres,movie["genre_ids"]);
...
const createCategory=(genre)=>{
    ...
    buttonGenre.addEventListener("click",async e=>{
        try {
            ...
            let creditsMoviesSelectedGenre=await getCreditsForMovies(moviesSelectedCategory);
            moviesSelectedCategory.forEach(movie=>{
                swiperWrappers[2].append(createMovie(movie,creditsMoviesSelectedGenre));
            })
            createSwiper();
...
searchButton.addEventListener("click",async e=>{
    ...
    try {
        ...
        let creditsForSearchedMovies= await getCreditsForMovies(searchedMovies);

        swiperWrappers[0].innerHTML="";
        searchedMovies.forEach(movie=>{
            swiperWrappers[0].append(createMovie(movie,creditsForSearchedMovies));
            createSwiper();
        });
...
const main=async()=>{
    ...
    actionMovies.forEach(movie=>{
        swiperWrappers[2].appendChild(createMovie(movie,creditsForActionMovies));
    })
